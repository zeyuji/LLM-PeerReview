# Acknowledgement:
# This code is based on or inspired by the following project.
...
def prune_cache(cache: Union[Tuple[Tuple[Tensor, Tensor]], DynamicCache], num_tokens_to_discard: int):
    ...
    if isinstance(cache, tuple):
        ...
        return prune_hybrid_cache(cache, num_tokens_to_discard)
    else:
...
def prune_hybrid_cache(cache: HybridCache, num_tokens_to_discard: int):
    """
    Prune the cache by removing the specified number of tokens from the end. This pruning works for models using DynamicCache.
    ...
    for layer in range(len(cache.key_cache)):
        cache.key_cache[layer][:, :, num_tokens_to_discard:, :] = 0.
        cache.value_cache[layer][:, :, num_tokens_to_discard:, :] = 0.
    return cache
