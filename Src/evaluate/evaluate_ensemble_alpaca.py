...
parser.add_argument(
    ...
    choices=range(4),
    default=None,
    help="Fixed model index for evaluating a single model. By default, read selected_model from response_dir.",
)
...
def main(args):
    ...
    if args.model_index is None:
        selected_models = load_selected_model(args.response_dir, expected_indices=sample_indices)
    else:
        selected_models = [args.model_index] * len(sample_indices)
